...
int main(int argc, char *argv[])
{
  ...
  auto ignoreArray = node->declare_parameter("ignore_array", "");
  ...
  float angleMax = node->declare_parameter("angle_max", 180.f);  // Maximum angle in degrees
  float angleMin = node->declare_parameter("angle_min", -180.f); // Minimum angle in degrees
  float rangeMax = node->declare_parameter("range_max", 64.f);   // Maximum range in meters
  float rangeMin = node->declare_parameter("range_min", 0.01f);  // Minimum range in meters
  ...
  if (!ret)
  {
    RCLCPP_ERROR(node->get_logger(), "Fail to initialize %s\n", laser.DescribeError());
    return -1;
  }
  ...
  if (!ret)
  {
    RCLCPP_ERROR(node->get_logger(), "Fail to start %s\n", laser.DescribeError());
    return -1;
  }
